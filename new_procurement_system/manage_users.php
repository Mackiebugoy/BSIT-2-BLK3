...
<main class="flex-1 ml-64 p-6">
    <!-- Title -->
    <h1 class="text-3xl font-semibold text-gray-800 dark:text-gray-200 mb-6">Manage Users</h1>
    ...
    <div class="overflow-x-auto bg-white dark:bg-gray-700 p-6 rounded-lg shadow-lg">
        <table class="w-full table-auto text-left border-collapse border border-gray-300">
            <thead>
                <tr>
                    <th class="border p-3 text-sm text-gray-700 dark:text-gray-300">Username</th>
                    <th class="border p-3 text-sm text-gray-700 dark:text-gray-300">Email</th>
                    <th class="border p-3 text-sm text-gray-700 dark:text-gray-300">Role</th>
            ...
            <tbody>
                <?php while ($user = $users->fetch_assoc()): ?>
                    <tr class="transition-all duration-300 hover:bg-gray-200 dark:hover:bg-gray-600">
                        <td class="border p-3"><?php echo $user['username']; ?></td>
                        <td class="border p-3"><?php echo $user['email']; ?></td>
                        <td class="border p-3"><?php echo ucfirst($user['role']); ?></td>
                        <td class="border p-3">
                            <a href="suspend_user.php?id=<?php echo $user['id']; ?>" 
                               class="text-red-500 hover:text-red-700 font-medium transition-colors">Suspend</a> | 
